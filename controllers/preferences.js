...
const getPreference = async (req, res) => {
  try {
    ...
    let data = await Preference.findById(_id);
    if (data === null) {
...
const updatePreference = async (req, res) => {
  try {
    const _id = req.params.id;
    let data = await Preference.findByIdAndUpdate(_id, req.body, {
      new: true,
    });
    if (data === null) {
      return res.status(200).json({ error: "id not found" });
...
const deletePreference = async (req, res) => {
  try {
    const _id = req.params.id;
    let data = await Preference.findByIdAndDelete(_id);
    if (data === null) {
      return res.status(200).json({ error: "id not found" });
    }
    res.status(200).json(data);
...
const getAllPreference = async (req, res) => {
  try {
    ...
    let apiData = Preference.find(queryObject);
